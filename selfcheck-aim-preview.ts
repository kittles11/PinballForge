 ...
 *       ② segHitsCircle 线段-圆扫掠单元 ③ LauncherController 接线（钉子快照收集、力竭过滤、
 *       初速倍率、重力真值、命中圈绘制、旧直线虚线常量已移除）
...
const aimPreviewSrc = read('Core', 'AimPreview.ts');

check('AimPreview 保持零 cc 依赖（自检可动态 import 真跑）', !aimPreviewSrc.includes("from 'cc'"));
check('drawTrajectory 改调 simulateAimPreview（旧直线虚线已移除）',
    /simulateAimPreview\(\{/.test(launcher) && !launcher.includes('DASH_LENGTH'));
check('初速 = launchSpeed × previewSpeedScale（对齐 fireOrb 的 velocity+impulse 双重叠加）',
    /this\.launchSpeed \* this\.previewSpeedScale/.test(launcher) && /previewSpeedScale = 2;/.test(launcher));
check('重力取物理系统真值（PhysicsSystem2D.gravity.y，异常回退 -320）',
    /PhysicsSystem2D\.instance\?\.gravity\?\.y \?\? -320/.test(launcher));
check('TOUCH_START 刷新钉子快照（拖拽期间钉子不变，收集一次）',
    /private onTouchStart\(event: EventTouch\): void \{[\s\S]*?this\.refreshPegSnapshot\(\);/.test(launcher));
check('快照收集全场钉子且过滤力竭钉（Collider 已禁用，物理上不存在）',
    /getComponentsInChildren\(PegComponent\)/.test(launcher) && /c\.isExhausted\) continue;/.test(launcher));
check('钉子半径取 CircleCollider2D 真值（异常回退 16）',
    /getComponent\(CircleCollider2D\)/.test(launcher) && /col \? col\.radius : ORB_RADIUS/.test(launcher));
check('钉子世界坐标换入 trajectoryGraphics 本地系（与轨迹同坐标系）',
...
check('松手/弹窗/终局仍清除预测线（clearTrajectory 保留且被调用）',
    /private clearTrajectory\(\): void/.test(launcher)
    && (launcher.match(/this\.clearTrajectory\(\)/g) || []).length >= 4);
