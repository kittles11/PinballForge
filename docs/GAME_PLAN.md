...
| 维度 | 已有 | 缺口 |
|---|---|---|
| 玩法 | 5 球种、3 漏斗、4 钉种、5 敌种、16 卡（含 3 机制应答卡）+ 3 跨局解锁卡、5 遗物、Boss 行为+精英词缀、5 钉板版型（2 解锁）、50 章×10 关、Meta 解锁树（10 轨三条根深度链）死亡补偿 | 动态难度、每日任务、长线目标 |
...
| D1 次留 | Meta 10 条×5 级三条根深度链（castle→{orbCap→boardLab, startShield} / gold→shard / damage→{orbLab, insight→forbiddenPack}），已含版型 D/E + 禁忌卡 + 第 5 球种（等离子球）跨局解锁 | 真机校准 + 视需要补数值轨/新球种变体 | 随机+固定奖励混合建立信任 |
