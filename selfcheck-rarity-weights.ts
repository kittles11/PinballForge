...
check('卡库构成：13 张 = 普通3 / 稀有7 / 史诗3（史诗=三张球种 AddOrb 卡）',
    CARD_DATABASE.length === 13 && byRarity('普通') === 3 && byRarity('稀有') === 7 && byRarity('史诗') === 3);
...
check('rng 恒 0.9999 → 逐轮取末张：普通救急卡逆序 [univ_gold_get, univ_hp, univ_heal]',
    hi.map((c) => c.id).join(',') === 'univ_gold_get,univ_hp,univ_heal');
...
// 牌库满（canAddOrb=false）时 AddOrb 整层被滤：AddOrb 共 4 张（lava_orb/lightning_split/lightning_rage/frost_orb）→ 池剩 9 张
const noOrbPool = CARD_DATABASE.filter((c) => c.actionType !== 'AddOrb');
const degraded = drawWeightedCards(noOrbPool, 3);
check('AddOrb 层滤空自动退化：9 张池抽 3 不崩、无 AddOrb 混入',
    noOrbPool.length === 9 && degraded.length === 3 && degraded.every((c) => c.actionType !== 'AddOrb'));

// ── ③ 蒙特卡洛频率 ──
// 单张精确锚点：P(普通)=300/625=0.48 / P(稀有)=280/625=0.448 / P(史诗)=45/625=0.072
const TRIALS = 20000;
...
check(`单抽 P(普通) ≈ 0.48（实测 ${pN1.toFixed(4)}，±0.01）`, Math.abs(pN1 - 0.48) < 0.01);
check(`单抽 P(稀有) ≈ 0.448（实测 ${pR1.toFixed(4)}，±0.01）`, Math.abs(pR1 - 0.448) < 0.01);
check(`单抽 P(史诗) ≈ 0.072（实测 ${pE1.toFixed(4)}，±0.01）`, Math.abs(pE1 - 0.072) < 0.01);
