...
check('霜冻冰球与漏斗解耦：入任意槽冰封全场（Task 008 寒霜导热：仅冰槽加时，其余槽不加时不限定）',
    /if \(this\.orbType === OrbType\.Frost\) \{\s*const duration = OrbBalance\.frost\.freezeDuration/.test(orbCtrl)
    && /this\.freezeAllEnemies\(duration\);/.test(orbCtrl)
...
check('极寒易伤乘区保留（冰封受伤 × iceVulnerableMult ×(1+freezeVulnerability)——2026-09-07 冰球签名易伤接线）',
    /this\.isFrozen\)\s*\{\s*dmg \*= EnemyController\.iceVulnerableMult \* \(1 \+ OrbBalance\.frost\.freezeVulnerability\);/.test(enemy));
