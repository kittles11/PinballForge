...
// Task 006 拆分：渲染契约（OrbBody 圆盘 / 拖尾 / 辉光 / 受击闪色）已迁至 OrbView
const view = strip(read('Pinball', 'OrbView.ts'));
...
check('四个选卡分支（降级 / 常规 / 传奇藏宝箱 / 锻造契约）的 playHideAnimation 全部位于 finally 块内（弹窗必关，看门狗不被 anyModalOpen 挡死）',
    (reward.match(/finally\s*\{\s*this\.playHideAnimation\(index\)/g) ?? []).length === 4);
check('_selecting = true 后必有 finally 收尾路径（降级/常规/藏宝箱/契约四处，异常时也不永久锁死选卡）',
    (reward.match(/this\._selecting = true;/g) ?? []).length === 4
...
check('MotionStreak 挂 OrbTrail 子节点（与 Sprite 同节点互斥；曾直接挂弹珠根节点，每次发射刷 renderable 警告且拖尾不渲染）',
    /getChildByName\('OrbTrail'\)/.test(view) && !/this\.addComponent\(MotionStreak\)/.test(view));
...
check('三处辉光 Sprite 改用引擎原生混合因子（srcBlendFactor/dstBlendFactor，_updateBlendFunc 在材质实例上正确叠加）',
    (view.match(/srcBlendFactor = gfx\.BlendFactor\.SRC_ALPHA/g) ?? []).length === 2
...
check('球体本体由 OrbBody 子节点 Graphics 实心圆盘绘制（同构根修；子节点规避 renderable 互斥）',
    /getChildByName\('OrbBody'\)/.test(view) && /g\.fillColor = this\._tint;\s*g\.circle\(0, 0, r\);/.test(view));
check('受击闪色迁至 OrbView._tint → redrawBody（disc 方案废弃；OrbController 零渲染残留）',
    !/RuntimeTex\.disc/.test(orb) && !/bodySp/.test(orb) && !/redrawBody/.test(orb)
    && !/RuntimeTex\.disc/.test(view) && !/bodySp/.test(view)
    && /this\._tint\.set\(flashColor\);\s*this\.redrawBody\(\);/.test(view));
...
// ── 7. 2026-09-07 两步确认选卡 + 全普通初始卡组（用户拍板回归钉）──
const deckSrc = strip(read('Core', 'DeckManager.ts'));
check('选卡两步确认：点卡只 markPendingSelection（高亮预选），selectReward 仅由确认按钮链路调用',
    /private markPendingSelection\(index: number\): void/.test(reward)
    && /card\.on\(Node\.EventType\.TOUCH_END, \(\) => \{\s*this\.markPendingSelection\(index\);/.test(reward)
    && (reward.match(/this\.selectReward\(/g) ?? []).length === 1); // 唯一调用方：onConfirmClick
check('确认按钮：ensureConfirmBtn 幂等创建 + 无预选/结算中拒点 + 新弹窗 clearPendingSelection 复位',
    /private ensureConfirmBtn\(\): void/.test(reward)
    && /if \(this\._pendingIndex < 0 \|\| this\._selecting\) \{\s*return;/.test(reward)
    && /this\.clearPendingSelection\(\);/.test(reward));
check('初始卡组全普通（INITIAL_DECK_TYPES 六颗 ORB_TYPE_NORMAL，特殊球只经商店 / 奖励获得）',
    (deckSrc.match(/INITIAL_DECK_TYPES: number\[\] = \[\s*ORB_TYPE_NORMAL, ORB_TYPE_NORMAL, ORB_TYPE_NORMAL,\s*ORB_TYPE_NORMAL, ORB_TYPE_NORMAL, ORB_TYPE_NORMAL,\s*\];/) ?? []).length === 1);
