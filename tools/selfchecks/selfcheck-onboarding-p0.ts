...
// ── ④ 漏斗常驻标注（2026-09-07 中性化改版：漏斗去主题色，EMOJ+文案标识）──
check('FunnelSlot.start 幂等创建类型标注 ensureTypeLabel',
    /this\.ensureTypeLabel\(\);/.test(funnel)
    && /getChildByName\('TypeLabel'\)/.test(funnel));
check('标注文案：💥 聚能 ×2 / ❄️ 精炼 ×1.5 / 💰 金币 +20（EMOJ 标识）',
    /💥 聚能 ×2/.test(funnel) && /❄️ 精炼 ×1\.5/.test(funnel) && /💰 金币 \+20/.test(funnel));
check('漏斗中性化：标注白色 + 正下方 42px；主题色语言已整体移除（无光柱/无 funnelColor）',
    /label\.color = Theme\.white/.test(funnel) && /setPosition\(0, -42, 0\)/.test(funnel)
...
check('baseHp 复合成长：线性基线 × HP_CHAPTER_GROWTH^(章-1)，系数锁定 1.030（防 1.15 指数爆炸回归；2026-09-07 校准自 1.045）',
    /const baseHp = Math\.round\(linearHp \* Math\.pow\(HP_CHAPTER_GROWTH, this\.currentChapter - 1\)\);/.test(level)
    && /const HP_CHAPTER_GROWTH = 1\.030;/.test(level));
...
// 2026-09-07 随 HP_CHAPTER_GROWTH 1.045→1.030 校准同步重算）
const hp = (ch: number, lv: number) => Math.round((140 + (ch - 1) * 90 + (lv - 1) * 12) * Math.pow(1.030, ch - 1));
check('锚点：1-1 普通怪 = 140', hp(1, 1) === 140);
check('锚点：5-10 精英 = 1710', Math.round(hp(5, 10) * 2.5) === 1710);
check('锚点：10-10 Boss = 6210', Math.round(hp(10, 10) * 4.5) === 6210);
check('锚点：50-10 Boss = 89213', Math.round(hp(50, 10) * 4.5) === 89213);
